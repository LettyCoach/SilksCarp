            ...
            <div class="card">
                <div class="panel-heading">
                    <div class="d-flex justify-content-between flex-wrap items-center mb-2 mt-4">
                        ...
                        <div class="rounded-md">
                            {{-- <select name="categories" class="form-select" id="categories" onchange="viewIndex()">
                                <option value="-1" selected>全て(種別)</option>
                                @foreach ($helpCategories as $k => $v)
                                    <option value="{{ $k }}" {{ $k == $categories ? 'selected' : '' }}>
                                        {{ $v }}</option>
                                @endforeach
                            </select> --}}
                                    ...
                                    @foreach ($models as $i => $model)
                                        <tr class="align-middle">
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('help.edit', ['help' => $model->id]) }}">
                                                    <i class="bi-pencil-square"></i>
                                                </a>
                                            </td>
                                            <td class="text-center">
                                                <form method="POST"
                                                    action="{{ route('help.destroy', ['help' => $model->id]) }}">
                                                    @csrf
                                                    @method('DELETE')
...
@section('js')
    ...
    <script>
        const indexUrl = "{{ route('help.index') }}";
        const viewIndex = () => {
